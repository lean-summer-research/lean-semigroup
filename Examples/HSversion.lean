...
/- note in the proof below (a) If we did not use WithOne, we would be faced with multiple cases, and the proof would be much longer. (b) We canalso use the abbreviated notation ↑ for WithOne.coe.  Since this can be ambiguous, it doesn't always work, but it saves typing when it does. -/
...
Now define R equivalence using the general tools for preorder the Setoid class. -/
...
In this example we'll construct the semigroup of all maps from a three-element set into itself, and construct some elements of the semigroup.  Then verify some inequalities and equivalences using the R order and R equivalence.
...
lemma ROrder_verify1 :  h ≤ᵣ f := by
  have rtmult : ∃ s : threemaps, h = f * s := by
    use g
    rfl
  have rlealt : R_le_alt h f := Or.intro_left _ rtmult
  exact (R_le_alt_Rle h f).mpr rlealt

#eval threemaps_prod h g

lemma verify_sublem: f = threemaps_prod h g := List.ofFn_inj.mp rfl
...
lemma ROrder_verify2 : (@ROrder threemaps).le f h := by
  have ex : ∃ x, f = h * x := by
    use g
    exact verify_sublem
  have rlealt: R_le_alt f h := Or.intro_left _ ex
  exact (R_le_alt_Rle f h).mpr rlealt
